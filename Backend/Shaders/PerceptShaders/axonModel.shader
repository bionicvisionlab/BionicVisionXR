               ...
               for(int m = axon_idx_start; m < axon_idx_end; m++){ // for each axon segment affecting this pixel...
                    ...
                    for(uint j=0; j<numberElectrodes; j++){ // for each electrode
                            if(electrodesBuffer[j].current > electrodeThreshold){
                                gauss = axonSegmentGaussToElectrodesBuffer[(m*numberElectrodes*numberRastGroups) + electrodesBuffer[j].electrodeNumber]; 
                                segment_brightness = segment_brightness + gauss * amplitude * electrodesBuffer[j].current * axonContributionBuffer[m].brightnessContribution;
                            }
                    }
