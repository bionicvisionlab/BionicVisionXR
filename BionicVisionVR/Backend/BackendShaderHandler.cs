...
public class BackendShaderHandler : MonoBehaviour {
    ...
    private void SetShaderVariables() {
        ...
        vm.perceptShaderMaterial.SetInt("frameDur", (int)pulseTrain.timeStepsPerFrame);
    ...
    private void GetGazeScreenPos() {
        ...
        var interpolatedGazeDirection = Vector3.Lerp(_lastGazeDirection, eyeTrackingData.GazeRay.Direction,
            vm.smoothMoveSpeed * Time.unscaledDeltaTime);
        var usedDirection = vm.smoothMove
            ? interpolatedGazeDirection.normalized
            : eyeTrackingData.GazeRay.Direction.normalized;
        _lastGazeDirection = usedDirection;

        screenPos = vm.gazeLock
            ? vrCamera.WorldToScreenPoint(vrCamera.transform.position + vrCamera.transform.rotation * usedDirection)
    ...
    private void BlurFinal() {
        if(vm.blurFinalImage){
            vm.blurShader.SetInt("_KernelSize", vm.blurIntensity);
            temp = processedTexture;
    ...
    public void UpdateAfterFrames() {
      ...
      private void UpdateConfiguration()
      {
          ...
          SetRandomizerArray();
          TaskHandler.Instance.trialTimer = Time.realtimeSinceStartup; }
    ...
    private void Awake() {
        ...
        simulationVariablesBuffer = new ComputeBuffer(10,
            System.Runtime.InteropServices.Marshal.SizeOf(typeof(SimulationVariables)));
