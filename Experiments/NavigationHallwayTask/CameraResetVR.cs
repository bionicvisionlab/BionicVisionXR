...
public class CameraResetVR : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((this.transform.position.z > 9.1 && HallwayTaskController.Instance.currentTrial%2==0) || (this.transform.position.z < -9.1 && HallwayTaskController.Instance.currentTrial%2==1))
        {
            HallwayTaskController.Instance.roundStarted = false;
            parentTransform.transform.Rotate(new Vector3(0f, 180.0f, 0f)); 
            
            fileHandler.AppendLine(HallwayTaskController.Instance.subjectFile, HallwayTaskController.Instance.currentBlock + ", " + (HallwayTaskController.Instance.currentTrial+1) + ": " + (Time.time - HallwayTaskController.Instance.roundTimer -.5) + ", " + HallwayTaskController.Instance.collisionCounter );
            fileHandler.AppendLine(HallwayTaskController.Instance.subjectFile.Replace(".csv","_head.csv"), HallwayTaskController.Instance.currentBlock + ", " + (HallwayTaskController.Instance.currentTrial+1) );
            fileHandler.AppendLine(HallwayTaskController.Instance.subjectFile.Replace(".csv","_head-position.csv"), HallwayTaskController.Instance.currentBlock + ", " + (HallwayTaskController.Instance.currentTrial +1) );
            fileHandler.AppendLine(HallwayTaskController.Instance.subjectFile.Replace(".csv","_head-position-easy-paste.csv"),  HallwayTaskController.Instance.currentBlock + ", " + (HallwayTaskController.Instance.currentTrial+1) );
            ...
            if (HallwayTaskController.Instance.currentTrial == HallwayTaskController.Instance.numTrials)
            {
                ...
                else
                {
                    ...
                    VariableManagerScript.Instance.predefinedSettings = 
                        HallwayTaskController.Instance.blockSettings[HallwayTaskController.Instance.blockOrder[HallwayTaskController.Instance.currentBlock]];
                }
